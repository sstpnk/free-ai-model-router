...
def test_provider_access_report_uses_historical_runtime_statuses() -> None:
    ...
    assert "| limited | Limited Provider | подключен, но лимит | добавлен | — | 1 | 0 | 0 | 1 | 0 |" in report
...
def test_provider_access_report_distinguishes_client_restricted_models() -> None:
    provider = ProviderConfig(
        provider_id="client",
    ...
    assert "часть моделей только из клиента, часть требует баланс" in report
    expected = (
        "| client | Client Provider | часть моделей только из клиента, часть требует баланс | "
        "добавлен | — | 2 | 0 | 0 | 1 | 1 |"
    )
